...
struct PCKDTree 
{
	...
	:root(NULL)
	{}
	// Create an Insert Helper Function <iSH>
	void iH(PCNode<PointT>** root, uint depth, PointT point, int id) {
 		if(*root == NULL) 
 		{
 		...
 		} else 
 		{
 			...
 			if(curr_dim == 0)
 			{
 				if(point.x < (*root)->point.x)
 					iH(&((*root)->left), depth+1, point, id);
 				else
 					iH(&((*root)->right), depth+1, point, id);
 			} else if(curr_dim == 1) 
 			{
	...
	// Create a Radius Search Helper function <rSH>
	void rSH(PCNode<PointT>* root, uint depth, PointT point, float clusterTolerance, std::vector<int>& k_indices, std::vector<float>& k_sqr_distances)
	{
		if(root != NULL) {
			...
			if((pointC - clusterTolerance) < rootC)
				rSH(root->left, depth+1, point, clusterTolerance, k_indices, k_sqr_distances);
			if((pointC + clusterTolerance) > rootC)
				rSH(root->right, depth+1, point, clusterTolerance, k_indices, k_sqr_distances);
		}
	}
	void radiusSearch(PointT current_point, float clusterTolerance, std::vector<int>& k_indices, std::vector<float>& k_sqr_distances) 
	{
		rSH(root, 0, current_point, clusterTolerance, k_indices, k_sqr_distances);
	}
};
